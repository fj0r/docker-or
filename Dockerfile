...
RUN set -eux \
  ...
  ; wget -qO- https://github.com/arut/nginx-dav-ext-module/archive/refs/tags/${DAV_EXT_VERSION}.tar.gz | tar -zxf - \
  ...
  ; make \
  ; make install \
  ; cd .. && rm -rf openresty-${OPENRESTY_VERSION} nchan-${NCHAN_VERSION} nginx-dav-ext-module-${DAV_EXT_VERSION} \
